...
function testInterpolationMethod1(f,theta_rad,S_2D,interp_size,SimInputParams,showPlots)
    %TESTINTERPOLATIONMETHOD1 
        % S_2D              = E(f, theta) where theta is in radians
        % f                 = frequency array
        % theta_rad         = direction array in radians
        % interp_size       = size of interpolated spectrum
    ...
    % Define a new simulated wave spectrum which is the same size as the
    % interpolated wave spectrum this is the IDEAL INTERP SPECTRUM
    f_2 = frequencyBins(interp_size, "ERA5");
    [theta_degrees_2, theta_rad_2] = directionBins(interp_size, ...
        SimInputParams(3), "ERA5");
    [S_1D_f_2, S_2D_2] = waveLibrary().simulateWaveSpectrum( ...
        SimInputParams(1),SimInputParams(4),f_2,SimInputParams(2),  SimInputParams(3), ...
        theta_degrees_2, 0, 'JONSWAP');
    
    % Calculate the wave number spectrum of the ideal interp spectrum
    w_2 = f_2.*(2*pi);
    k_2 = (w_2).^2 ./ 9.81;
    [E_kx_ky_2, ~, kx_matrix_2,ky_matrix_2] = waveLibrary().waveNumberSpectrum(S_2D_2,w_2,k_2,theta_rad_2);


    % Calculate the wave number spectrum of the input spectrum
    w = f.*(2*pi);
    k = (w).^2 ./ 9.81;
    [E_kx_ky, ~, kx_matrix,ky_matrix] = waveLibrary().waveNumberSpectrum(S_2D,w,k,theta_rad);
    
    % Interpolate the input wave number spectrum
    cw = w./k; % Calculate wave phase speed 
    cg = 0.5 .* cw; % Calculate group wave phase speed 
    R_k = k;
    E_w_theta_rad = S_2D ./ (2 * pi); % Convert to E(f,theta) --> E(w,theta)
    E_k_theta_rad = E_w_theta_rad .* cg; % Convert to E(w,theta) --> E(k,theta)
    [test_interp_E_kx_ky, X,Y] = polar_surface(R_k, theta_rad, E_k_theta_rad,1,1,interp_size);
    
    
    
    % Plot the surface
    figure;
    contour(X, Y, test_interp_E_kx_ky);
    colorbar;
    xlabel('X'); ylabel('Y');
    title('Polar Surface E(k, theta) in Cartesian Coordinates E(kx,ky)');
    axis equal;
    ...
    figure;
    contour(X, Y, test_interp_E_kx_ky.*cw./w);
    colorbar;
    xlabel('X'); ylabel('Y');
    title('Polar Surface E(k, theta) in Cartesian Coordinates E(kx,ky)');
    axis equal; 
