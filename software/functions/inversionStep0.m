function [J_step0,F_new, P_new, energy_0_i,new_kx_0,new_ky_0,rot_angle_0] = inversionStep0(nonlinearity_order, plotsON,transect_number, P_obs,first_guess_wave_number_spectrum,kx,ky,first_guess_omega,first_guess_k,sar_sub_transect_size, cosmo, handh)
    ...
    for i_energy = 1:length(energy)
        for i_mag = 1:length(wave_number_mag)
            for i_rot = 1:length(rot)
                ...
                % new_kx = imrotate(kx,theta,"bilinear","crop");
                % new_ky = imrotate(ky,theta,"bilinear","crop");
                % 
                % 
                % new_k = sqrt(new_kx.^2 + new_ky.^2);
                % gravity =9.81;
                ...
                P_tmp = handh.generateSARSpectrumFromWaveNumberSpectrum(cosmo, plotsON, transect_number, nonlinearity_order, sar_sub_transect_size, new_kx, new_ky, new_omega, new_k, S_tmp); % [Eq.65 HH1991]
