...
f = era5_freq_bins;
dirBins = era5_direction_bins_degrees;
theta_degrees = era5_direction_bins_degrees;
S_2D = era5_d2fd';
S_1D = trapz(dirBins .* (pi/180),S_2D,2);
...
S_2D_trimmed = S_2D(min_freq_index:max_freq_index,:);
S_2D_trimmed(~isfinite(S_2D_trimmed)) = 0; % Remove NaN values
S_1D_trimmed = trapz(dirBins .* (pi/180),S_2D_trimmed,2);
...
% Compute Initial Significant Wave Height
Hs_trimmed_init = 4 * sqrt(trapz(f_within_SAR_range, trapz(dirBins .* (pi / 180), S_2D_trimmed, 2), 1))
